...
def getDate(dateVal):
    ...
    return " ".join ( dateVal )  # return datetime.datetime.strptime(' '.join(dateVal), '%d %b %Y')
...
def parsergedcomfile(filepath):
    # Tags which are relevant
    taglist = [ 'INDI' , 'NAME' , 'SEX' , 'BIRT' , 'DEAT' , 'FAMC' , 'FAMS' , 'FAM' , 'MARR' , 'HUSB' , 'WIFE' ,
                'CHIL' , 'DIV' ,
                'DATE' ,
                'HEAD' , 'TRLR' , 'NOTE' ]
    # Tags necessary for parsing
    readable = [ 'INDI' , 'NAME' , 'SEX' , 'BIRT' , 'DEAT' , 'FAMC' , 'FAMS' , 'FAM' , 'MARR' , 'HUSB' , 'WIFE' ,
                 'CHIL' , 'DIV' ,
                 'DATE' ]
    # Holds the complete parsed file
    maindict = {}
            ...
            if len ( dat ) >= 2:
                ...
                if dat[ 0 ] == '0':
                    ...
                    if dat[ 1 ] in readable:
                        ...
                        prev.append ( False )
        filehandler.close ( )
        createresponse ( response , maindict , filepath )
        return maindict
    except IOError:
        return 'This file does not exist.'


def createresponse(response , maindict , fname):
    # List of people with their details to response
    response += "People:\n"
    response += "Id".ljust ( 10 ) + 'Name'.ljust ( 30 ) + 'Gender'.ljust ( 10 ) + 'Birthday'.ljust (
        15 ) + 'Alive'.ljust (
        10 ) + 'Death'.ljust ( 15 ) + 'Spouse'.ljust ( 20 ) + 'Children\n'
    ...
    for key in sorted ( maindict[ 'FAM' ] , key=lambda x: int ( x.replace ( '@' , "" ).replace ( 'F' , "" ) ) ):
        ...
        if 'HUSB' in maindict[ 'FAM' ][ key ]:
            response += (
                maindict[ 'FAM' ][ key ][ 'HUSB' ][ 'VAL' ] + ", " +
                maindict[ 'INDI' ][ maindict[ 'FAM' ][ key ][ 'HUSB' ][ 'VAL' ] ][ 'NAME' ][
                    'VAL' ]).ljust ( 60 )
        else:
            response += 'N/A'.ljust ( 40 )
        if 'WIFE' in maindict[ 'FAM' ][ key ]:
            response += (
                maindict[ 'FAM' ][ key ][ 'WIFE' ][ 'VAL' ] + ", " +
                maindict[ 'INDI' ][ maindict[ 'FAM' ][ key ][ 'WIFE' ][ 'VAL' ] ][ 'NAME' ][
                    'VAL' ]).ljust ( 60 )
        else:
            response += 'N/A'.ljust ( 40 )
        if 'CHIL' in maindict[ 'FAM' ][ key ]:
