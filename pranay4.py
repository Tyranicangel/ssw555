	...
	for count in out['INDI']:		
		...
		if 'BIRT' in out['INDI'][count]:
			...
			if age < 0:
				...
				response += 'ERROR: US07: - Future birth date of individual-id -' + count
				...
				response += 'ERROR: US07: - Age is more than 150 years, individual-id ' + count
		#print birthdict, '\n'
				...
				for x,y in birthdict.iteritems():
					if x == count: #If Individual-id's are same then get birth date 
						...
						if deathval.date() > today.date(): #Check if death date is a future date.
							...
							response 'Error: US07: Death date is future date - ' + str(deathval.date()) + ' for individual-id ' + count + '\n'
						else:
							...
							if age >= 0 and age < 150:
								...
								#print '\nDEATH:: -->Success, Individual id in DD-> ' + count + ' and Death year - ', (deathval.year), '\n'
					 			...
					 			response 'Error: US07: - Death before birth or future birth date of individual-id ' +count + '\n'
	return response
